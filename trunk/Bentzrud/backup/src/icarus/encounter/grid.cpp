...
{
    math::vector2f world_pos = math::vector2f(col * (100) +
                                            (row % 2 ? 50 : 0) + GRID_X_,
                                      row * (60) + GRID_Y_);
...
grid_tile* grid::get_tile_at(const math::vector2i& vec_pos) const
{
    return get_tile_at(vec_pos.y, vec_pos.x);
}
...
void grid::handle_mouse_over(math::vector2i mouse_xy)
{
    ...
    sf::FloatRect grid_rect(sf::Vector2f(GRID_X_ - 50, GRID_Y_ - 40), sf::Vector2f(
...
void grid::get_path_to_tile(const math::vector2i& target_tile,

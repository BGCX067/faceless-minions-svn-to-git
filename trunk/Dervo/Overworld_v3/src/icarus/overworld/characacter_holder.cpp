...
void characacter_holder::setup_character(hex_sprite* spawn_point, utilities::yth_node* data)
{
    ...
    //setup encounter
    encounter_list.setup_data_link(data);
}
...
void characacter_holder::update_character(nation_creator& nations, std::vector<nation_info>& n_info) // skal oppdateres hver frame
{
                 ...
                 unsigned max_size)
{
            ...
            num_tiles++)
    {
        ...
        if(this_size < max_size)
            runn_trough(target_tile->get_surrounding_tile(num_tiles), type, this_size+1, max_size);
    }
}
} // namespace overworld
} // namespace icarus

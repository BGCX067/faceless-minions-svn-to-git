...
void tooltip_handler::update_tooltip(const sf::Event& move_event)
{
    ...
    {
        if (base_tooltip::get_list()[i]->is_enabled() &&
            base_tooltip::get_list()[i]->get_hover_bounds().contains(position_.get_sf_vector()))
        {
...
void tooltip_handler::create_tooltip(base_tooltip* tooltip_ptr)
{
    ...
    if (position_.y + height > state_manager::get()->get_dimensions().y)
    {
        position_.y -= height;
    }
    if (position_.x + width > state_manager::get()->get_dimensions().x)
    {
        position_.x -= width;
    }
...
void tooltip_handler::set_status_all(bool status)
{
    for (unsigned i = 0; i < base_tooltip::get_list().size(); ++i)
    {
        base_tooltip::get_list()[i]->set_enabled(status);
    }
}

...
{
    data.current_health_ = static_cast<float>(current_health_) / static_cast<float>(health_);
    data.kills_ = kills_;
...
void character_hero::setup_skills(const std::string& char_class, const unsigned (&levels)[4])
{
    ...
    active_skills_[0].set_autocast(true);
}
...
sf::Sprite* character_hero::get_portrait_ptr()
{

...
bool character_npc::check_los_to_target()
{
    if (!skill_target_)
        return false;
    math::vector2f a2(skill_target_->get_world_pos());
    ...
    {
        if (distance == 0 ||
            (intersections[i]->get_range_from(this->get_vector_pos()) != 0 &&
             intersections[i]->get_range_from(this->get_vector_pos()) < distance))
        {
            distance = intersections[i]->get_range_from(this->get_vector_pos());
            closest = intersections[i];
        }
    }
    if (closest != NULL &&
        closest == skill_target_)
        return true;
    return false;
...
//           grid_ptr_->get_bounds().contains(pos))
//    {
//        pos += offset;
//        if (current_tile == NULL ||
//            !current_tile->get_bounds().contains(pos))
//        {
...
ai::status::value character_npc::select_skill()
{
    ...
    for (unsigned i = 0; i < skill_count_; ++i)
    {
        if (skills_[i].is_ready() &&
            (ranged_ammo_ > 0 || skills_[i].get_range() <= 1))
            selected_skill_ = i;
    }

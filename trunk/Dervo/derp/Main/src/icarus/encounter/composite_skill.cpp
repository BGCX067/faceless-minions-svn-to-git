...
void composite_skill::setup_from_data(utils::yth_node* skill_node)
{
    ...
    for (unsigned i = 0; i < skill_node->child_count("sfx"); ++i)
            sound::get()->load_sfx(sfx_node->child(skill_node->child("sfx", i)->value())->value(), skill_node->child("sfx", i)->value());
    for (unsigned k = 0; k < skill_node->child_count("subskill"); ++k)
    {
        ...
        if (skill_node->child("subskill", k)->child_count("projectile") > 0)
        {
            ...
            if (projectile_node->child_count("impact") > 0)
                for (unsigned i = 0; i < projectile_node->child("impact")->child_count("sfx"); ++i)
                    sound::get()->load_sfx(sfx_node->child(projectile_node->child("impact")->child("sfx", i)->value())->value(), projectile_node->child("impact")->child("sfx", i)->value());
        }
    }
}

...
void state_manager::initialize(const std::string& config_path)
{
    ...
    uint32_t s_width = (config_->get_value("full_hd", false) ? 1920 : 1280);
    uint32_t s_height = (config_->get_value("full_hd", false) ? 1080 : 720);
    full_hd_ = config_->get_value("full_hd", false);
    window_ = new sf::RenderWindow(sf::VideoMode(s_width, s_height),

...
void party_member::setup_dialog(unsigned party_index)
{
    ...
    for (unsigned i = 0; i < 4; ++i)
    {
        tex_path = "images/interface/icons/"+icon_root->child(class_node->child("skill", i+1)->value())->value();
        if ((tex_ptr = resource_handler::get()->get_texture(tex_path)))
            skills_[i].set_texture(*tex_ptr);
        tex_ptr = NULL;
        skills_[i].set_active((selected_member_->get_skill_lvl(i) > 0));
        skills_[i].set_highlight((selected_member_->get_skill_pts() > 0 &&
                                  selected_member_->get_skill_lvl(i) < (i < 3 ? 3 : 1)));
    ...
    for (unsigned i = 0; i < 4; ++i)
    {
        inventory_[i].setPosition(base_pos + offset + (math::vector2f(0.0f, spacing) * i));
    }
    ...
    for (unsigned i = 0; i < 4; ++i)
    {
        ...
        autocast_indicators_[i]->set_world_pos(base_pos + math::vector2f(offset.x, (i%2?alt_x:offset.y)) + (math::vector2f(spacing, 0.0f) * i) + math::vector2f(5.0f, 5.0f));
    }

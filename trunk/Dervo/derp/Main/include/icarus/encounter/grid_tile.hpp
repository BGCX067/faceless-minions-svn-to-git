...
public:
    ...
    sf::FloatRect get_bounds() const { return tile_.get_bounds(); }
    void set_occupant(character_base* occupant);
    character_base* const get_occupant() const { return occupant_; }
    tile_state::state get_state() const
    {
         return out_of_bounds_ && current_state_ == tile_state::unoccupied ? tile_state::out_of_bounds : current_state_;
    }
    void set_state(tile_state::state new_state);
    ...
    void reserve_tile()
    {
        ...
        current_state_ = tile_state::reserved;
    }
    void unreserve();

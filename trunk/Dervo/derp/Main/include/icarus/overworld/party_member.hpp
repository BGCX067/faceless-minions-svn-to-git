...
public:
    ...
    float get_current_hp() const { return float(current_health_) / base_health_; }
    int get_current_hp_abs() const { return current_health_; }
    void heal(float amount) { current_health_ = math::clamp(int(current_health_ * (1.0f + amount)), 0, base_health_); }

...
namespace overworld
{
nation_info::nation_info(overworld::nation type_nation, typedef_tile& tiles) :
            type_nation_(type_nation),
            nation_tiles_(tiles)
{
    //ctor
    //for (unsigned i = 0; i < tiles.size(); ++i)
    //   nation_tiles_.push_back(tiles[i]);
}
...
void nation_info::generate_campaign()
{
    ...
    {
    ////std::cout << "      tile number is: " << number << std::endl;
    ////std::cout << "        get.surounding_tile_length is: " << (*nation_tiles_[number]).get_surrounding_tile_length() << std::endl;
...
void nation_info::next_step()
{
    ...
    if(siege_zone_.is_active())
    {
        if(siege_zone_.run_battle())
        {
            ...
            {
                //std::cout << "return_winner = false" << std::endl;
                generate_campaign();
            }
        }
    }

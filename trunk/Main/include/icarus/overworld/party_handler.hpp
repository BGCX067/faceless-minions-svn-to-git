...
namespace overworld
{
class party_handler : public utils::serializable
{
private:
    party_member* party_[6];
    utils::current_state state_;
    std::vector<utils::party_member> encounter_members_;
...
public:
    ...
    void get_party_data(encounter::data* data_ptr, bool reset_metrics = false) const;
    ...
    bool party_contains(encounter::hero_class::type type) const;
    unsigned highest_level_of_class(encounter::hero_class::type type) const;
    bool unspent_points() const;
    ...
    utils::current_state& get_state() { return state_; }
    std::vector<utils::party_member>& get_encounter_members() { return encounter_members_; }
    void reset_party_gold() { party_gold_ = 0; }
    const std::vector<std::string>& get_graveyard() const { return graveyard_; }
};
}
}

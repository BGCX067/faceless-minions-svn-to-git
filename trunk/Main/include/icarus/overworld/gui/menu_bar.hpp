...
{
private:
    sf::Sprite background_left_;
    sf::Sprite background_right_;
    struct button
    {
        sf::Sprite graphic_;
        sf::Text text_;
        ...
        void set_position(const math::vector2f& pos)
        {
            ...
            text_.setPosition(pos+((math::vector2f(graphic_.getLocalBounds().width, graphic_.getLocalBounds().height) -
                                   math::vector2f(text_.getLocalBounds().width, text_.getLocalBounds().height))/2).rounded());
        }
    };

...
protected:
    ...
    bool is_obstacle_;
    bool is_dead_;
    float dmg_fade_timer_;
    float dmg_fade_current_;
    float dmg_fade_factor_;
    unsigned minimum_damage_;
    unsigned maximum_damage_;
...
public:
    ...
    virtual float get_current_health() const { return current_health_; }
    ...
    unsigned get_weapon_dmg()
    {
        int rand_v = maximum_damage_-minimum_damage_;
        unsigned damage = minimum_damage_ + (rand_v <= 0 ? 0 : (rand()%(rand_v+1))) + stored_damage_;
        if (stored_damage_ > 0)
            stored_damage_ = 0;
        return damage;
    }
    ...
    void set_invisible(bool state) { render_ = !state; }

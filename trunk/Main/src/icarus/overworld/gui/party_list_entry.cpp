            ...
            math::vector2f(135.0f, 3.0f))
{
    ...
    tex_path = dialog_root->child("delete")->value();
    if ((tex_ptr = resource_handler::get()->get_texture(tex_path)))
        delete_ = sf::Sprite(*tex_ptr);
    ...
    tex_path = "images/ui/icons/icon_empty.png";
    tex_ptr  = NULL;
    if ((tex_ptr = resource_handler::get()->get_texture(tex_path)))
...
void party_list_entry::set_position(const math::vector2f& pos)
{
    ...
    delete_.setPosition(pos + math::vector2f(atof(node->child("delete_x")->value().c_str()),
                                           atof(node->child("delete_y")->value().c_str())));
}
}   // namespace gui
}   // namespace overworld
}   // namespace icarus

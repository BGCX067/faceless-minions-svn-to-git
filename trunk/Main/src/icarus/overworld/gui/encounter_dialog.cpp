...
void encounter_dialog::setup_dialog(utils::yth_node* encounter_node)
{
    ...
    for (unsigned i = 0; i < encounter_node->child("options")->child_count("option"); ++i)
    {
        ...
        new_option.loot_modifier_ = 0;
        new_option.loot_chance_ = 0;
        new_option.unique_chance_ = 0;
        ...
        if (encounter_node->child("options")->child("option", i)->child_count("loot_modifier") > 0)
        {
            new_option.loot_modifier_ = atoi(encounter_node->child("options")->child("option", i)->child("loot_modifier")->value().c_str());
        }
        if (encounter_node->child("options")->child("option", i)->child_count("loot_chance") > 0)
        {
            new_option.loot_chance_ = atoi(encounter_node->child("options")->child("option", i)->child("loot_chance")->value().c_str());
        }
        if (encounter_node->child("options")->child("option", i)->child_count("unique_chance") > 0)
        {
            new_option.unique_chance_ = atoi(encounter_node->child("options")->child("option", i)->child("unique_chance")->value().c_str());
        }
        ...
        else if(encounter_node->child("options")->child("option", i)->value() == "siege")
        {
            new_option.result_ = encounter_resolution::siege;
        }
        else if(encounter_node->child("options")->child("option", i)->value() == "siege")
        {
            new_option.result_ = encounter_resolution::siege_capital;
        }
...
void encounter_dialog::select_option(unsigned selected)
{
    ...
    resolved_ = true;
    party_handler::get()->get_state().result_ = unsigned(1);
    if (options_[selected_option_].chance_ < 1.0)
    {
        if (!((rand()%100)/100.0f > options_[selected_option_].chance_))
        {
            ...
            party_handler::get()->get_state().result_ = unsigned(0);
        }
    }
    reset_position();

    party_handler::get()->get_state().state_ = unsigned(utils::encounter_state::choice_made);
    party_handler::get()->get_state().selected_ = unsigned(selected_option_);
...
void encounter_dialog::close_dialog()
{
    ...
    case encounter_resolution::combat:
    {
        ...
        data_ptr->loot_modifier_ = options_[selected_option_].loot_modifier_;
        data_ptr->loot_chance_ = options_[selected_option_].loot_chance_;
        data_ptr->unique_chance_ = options_[selected_option_].unique_chance_;
        party_handler::get()->get_party_data(data_ptr, true);
    ...
    case encounter_resolution::fail:
    {
        if (combat_node_ != NULL)
        {
            ...
            data_ptr->scenario_count_ = combat_node_->child_count("scenario");
            data_ptr->experience_value_ = 0;
            data_ptr->loot_modifier_ = 0;
            data_ptr->loot_chance_ = 0;
            data_ptr->unique_chance_ = 0;
            party_handler::get()->get_party_data(data_ptr, true);
            state_manager::get()->store_data(data_ptr);
            state_manager::get()->push_state(states::encounter);
        }
    ...
    case encounter_resolution::siege:
    {
        encounter::data* data_ptr = new encounter::data();
        data_ptr->encounter_node_ = combat_node_;
        data_ptr->current_scenario_ = 0;
        ...
        data_ptr->loot_chance_ = options_[selected_option_].loot_chance_;
        data_ptr->unique_chance_ = options_[selected_option_].unique_chance_;
        data_ptr->is_siege_battle = true;
        party_handler::get()->get_party_data(data_ptr, true);
        state_manager::get()->store_data(data_ptr);
        state_manager::get()->push_state(states::encounter);

        std::cout << "SIEGE! or something. " << std::endl;

        //siege is sucsessfull. ?

        break;
    }
    case encounter_resolution::siege_capital:
    {
        encounter::data* data_ptr = new encounter::data();
        data_ptr->encounter_node_ = combat_node_;
        data_ptr->current_scenario_ = 0;
        ...
        std::cout << "SIEGE the capital. " << std::endl;
...
void encounter_dialog::load_dialog()
{
    utils::current_state state = party_handler::get()->get_state();
    switch(static_cast<utils::encounter_state::value>(party_handler::get()->get_state().state_))
    {
    case utils::encounter_state::no_encounter:
        break;
    case utils::encounter_state::enter_encounter:
        {
            utils::yth_node* encounter_node = resource_handler::get()->get_root_node("encounters");
            encounter_node = encounter_node->child(int(state.region_))->child(int(state.encounter_));
            setup_dialog(encounter_node);
        }
        break;
    case utils::encounter_state::choice_made:
        {
            utils::yth_node* encounter_node = resource_handler::get()->get_root_node("encounters");
            encounter_node = encounter_node->child(int(state.region_))->child(int(state.encounter_));
            setup_dialog(encounter_node);
            selected_option_ = int(state.selected_);
            resolved_ = true;
            if (int(state.result_) == 0)
            {
                options_[selected_option_].resolution_text_.setString(options_[selected_option_].alt_resolution_.getString());
                options_[selected_option_].result_ =  encounter_resolution::fail;
            }
        ...
        break;
    case utils::encounter_state::encounter_start:
        {
            utils::yth_node* encounter_node = resource_handler::get()->get_root_node("encounters");
            encounter_node = encounter_node->child(int(state.region_))->child(int(state.encounter_));
            setup_dialog(encounter_node);
        ...
        break;
    case utils::encounter_state::encounter_in_progress:
        break;
    case utils::encounter_state::encounter_resolved:
        break;
    }
}
}   // namespace gui

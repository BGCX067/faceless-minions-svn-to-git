    ...
    experience_value_(0)
{
    ...
    hp_bar_.set_label("");
    hp_bar_.set_size(math::vector2f(100.0f, 5.0f));
    is_ai_ = true;
    ...
    add(new ai::sequence_node())->
        ...
        add(new ai::encounter::task(*this, &character_npc::select_ammo_skill))->end()->
    ...
    add(new ai::sequence_node())->
        add(new ai::encounter::condition(*this, &character_npc::check_if_moving))->end()->
        add(new ai::encounter::condition(*this, &character_npc::check_targets_at_destination))->end()->
    end()->
    add(new ai::sequence_node())->
        add(new ai::reversal_node())->
            add(new ai::encounter::condition(*this, &character_npc::check_if_targets_in_range))->end()->
    ...
    behavior_result_.next_ = bt_root_;
    behavior_result_.result_ = ai::status::running;
    behavior_result_.down_ = true;
    //ctor
}
...
void character_npc::update(double delta)
{
    ...
    if (!incapacitated_)
    {
        if (behavior_result_.next_ == NULL)
        {
            behavior_result_.next_ = bt_root_;
            behavior_result_.down_ = true;
        }
        behavior_result_ = behavior_result_.next_->execute(behavior_result_);
    }
}
void character_npc::setup_from_data(utils::yth_node* data)
{
    ...
    skill_count_ = 0;
    experience_value_ = atoi(data->child("experience_value")->value().c_str());

    if (data->child_count("dodge_chance") > 0)
        dodge_chance_ = atoi(data->child("dodge_chance")->value().c_str());
    if (data->child_count("block_chance") > 0)
        block_chance_ = atoi(data->child("block_chance")->value().c_str());
    if (data->child_count("resist_chance") > 0)
        resist_chance_ = atoi(data->child("resist_chance")->value().c_str());
    if (data->child_count("crit_chance") > 0)
        critical_chance_ = atoi(data->child("crit_chance")->value().c_str());
    ...
    utils::yth_node* animation_node = sprite_node->child(data->child("sprite_sheet")->value())->child(data->child("sprite")->value());
...
bool character_npc::check_if_targets_in_range()
{
    ...
    int range = (selected_skill_ != -1 ? skills_[selected_skill_].get_range() : 1);
...
bool character_npc::check_targets_at_destination()
{
    math::vector2i target;
    if (path_to_target_.size() > 0)
        target = path_to_target_[0]->vec_pos;
    else
        target = vector_position_;
    std::vector<grid_tile*> tiles;
    grid_ptr_->get_tiles_in_radius(target,
                                   1,
                                   tiles);
    for (unsigned i = 0; i < tiles.size(); ++i)
    {
...
{
    selected_skill_ = -1;
    for (unsigned i = 0; i < skill_count_; ++i)
    {
        if (skills_[i].is_ready() &&
            (skills_[i].get_ammo_cost() == 0) &&
            skills_[i].get_range() > 0)
            selected_skill_ = i;
        else if (skills_[i].is_ready() && skills_[i].get_range() == 0)
        {
            std::vector<grid_tile*> tiles;
            grid_ptr_->get_tiles_in_radius(vector_position_, 1, tiles);
            unsigned foe_count = 0;
            for (unsigned k = 0; k < tiles.size(); ++k)
                foe_count += ((tiles[k]->get_occupant() != NULL && !tiles[k]->get_occupant()->is_ai()) ? 1 : 0);
            if (foe_count >= 2)
                selected_skill_ = i;
        }
    }
...
ai::status::value character_npc::select_ammo_skill()
{
    selected_skill_ = -1;
    for (unsigned i = 0; i < skill_count_; ++i)
    {
        if (skills_[i].is_ready() &&
            (ranged_ammo_ > 0 && skills_[i].get_ammo_cost() > 0) &&
            skills_[i].get_range() > 0)
            selected_skill_ = i;
    }
    return (selected_skill_ != -1 ? ai::status::success : ai::status::failed);
...
ai::status::value character_npc::select_target()
{
    if (selected_skill_ != -1)
    {
        ...
        for (unsigned i = 0; i < tiles.size(); ++i)
        {
            if (tiles[i]->get_occupant() != NULL && !tiles[i]->get_occupant()->is_ai())
            {
                if (!skills_[selected_skill_].requires_los())
                {
                    skill_target_ = tiles[i];
                    return ai::status::success;
                }
                math::vector2f a2(tiles[i]->get_world_pos());
                std::vector<grid_tile*> intersections;
                grid_ptr_->get_intersections(this->get_world_pos(), a2, intersections);
                unsigned distance = 0;
                grid_tile* closest = NULL;
                for (unsigned k = 0; k < intersections.size(); ++k)
                {
                    if (distance == 0 ||
                        (intersections[k]->get_range_from(this->get_vector_pos()) != 0 &&
                         intersections[k]->get_range_from(this->get_vector_pos()) < distance))
                    {
                        distance = intersections[k]->get_range_from(this->get_vector_pos());
                        closest = intersections[k];
                    }
                }
                if (closest != NULL && !closest->get_occupant()->is_ai())
                {
                    skill_target_ = tiles[i];
                    return ai::status::success;
                }

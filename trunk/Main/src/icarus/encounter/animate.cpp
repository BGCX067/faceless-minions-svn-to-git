...
void animate::update_anim()
{
    ...
    if(time_elapsed_ >= 1.0/static_cast<double>(target_framerate_))
    {
        ...
        if (current_frame_ >= active_animation_.frame_count_)
        {
            ...
            {
                one_shot_ = false;
                set_animation_state(prev_anim_state_);
            }
        }
...
{
    float scale = (state_manager::get()->get_hd_status() ? 1.0f : 0.66666f);
    if(positive_axis == false)
        character_sprite_->setScale(-scale, scale);
    else
        character_sprite_->setScale(scale, scale);
}
...
void animate::set_animation_state(const std::string& new_state, bool one_shot)
{
    // set the active animation and state
    if (anim_map_.count(new_state) <= 0 || animation_state_.compare(new_state) == 0 ||
        (one_shot_ && !one_shot))
        return;
    ...
    if (one_shot && !one_shot_)
    {
        one_shot_ = true;
        prev_anim_state_ = animation_state_;
    }

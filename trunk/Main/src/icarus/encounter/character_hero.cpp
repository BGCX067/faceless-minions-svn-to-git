...
void character_hero::update(double delta)
{
    ...
    if (favored_target_ != NULL && favored_target_->is_dead())
        favored_target_ = NULL;
}
...
void character_hero::setup_from_data(const hero_data& data, unsigned index)
{
    ...
    minimum_damage_ = data.base_min_damage_;
    maximum_damage_ = data.base_max_damage_;
    ...
    case hero_class::warrior:
        ...
        dodge_chance_ = class_values::WARRIOR_BASE_DODGE;
        block_chance_ = class_values::WARRIOR_BASE_BLOCK;
        resist_chance_ = class_values::WARRIOR_BASE_RESIST;
        critical_chance_ = class_values::WARRIOR_BASE_CRIT;
        break;
    case hero_class::mage:
        setup_skills("mage", data.skill_levels_, data.autocast_);
        class_node = class_node->child("mage");
        dodge_chance_ = class_values::MAGE_BASE_DODGE;
        block_chance_ = class_values::MAGE_BASE_BLOCK;
        resist_chance_ = class_values::MAGE_BASE_RESIST;
        critical_chance_ = class_values::MAGE_BASE_CRIT;
        break;
    case hero_class::rogue:
        setup_skills("rogue", data.skill_levels_, data.autocast_);
        class_node = class_node->child("rogue");
        dodge_chance_ = class_values::ROGUE_BASE_DODGE;
        block_chance_ = class_values::ROGUE_BASE_BLOCK;
        resist_chance_ = class_values::ROGUE_BASE_RESIST;
        critical_chance_ = class_values::ROGUE_BASE_CRIT;
        break;
    }
    ...
    dodge_chance_ += (attribute_mod::AGI_DODGE_MOD * agility_) + data.dodge_chance_;

    block_chance_ += data.block_chance_; // check for shields
    block_amount_ = 0.5f + data.block_mult_;

    resist_chance_ += (attribute_mod::INT_RESIST_MOD * intelligence_) + data.resist_chance_;
    resist_amount_ = 0.4f + data.resist_mult_;

    critical_chance_ += (attribute_mod::INT_CRITICAL_MOD * intelligence_) + data.crit_chance_;
    critical_mod_ = 1.0f + (attribute_mod::AGI_CRITICAL_MOD * agility_) + data.crit_mult_;

    phys_damage_mod_ = 1.0f + (attribute_mod::STR_DAMAGE_MOD * strength_) + data.damage_mod_;
    magic_damage_mod_ = 1.0f + (attribute_mod::INT_DAMAGE_MOD * intelligence_) + data.damage_mod_;
...
void character_hero::get_data(hero_data& data)
{
    ...
    data.largest_given_ = (largest_given_ > data.largest_given_ ? largest_given_ : data.largest_given_);
    data.largest_taken_ = (largest_taken_ > data.largest_taken_ ? largest_taken_ : data.largest_taken_);

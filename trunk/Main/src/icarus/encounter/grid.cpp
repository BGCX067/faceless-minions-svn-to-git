...
{
grid::grid() :
    GRID_X_(utils::to_hd(40.0f)),
    GRID_Y_(utils::to_hd(250.0f)),
    row_count_(0),
    col_count_(0),
    ...
    previous_tile_(NULL)
{
    tile_width_ = utils::to_hd(tile_width_);
    tile_height_ = utils::to_hd(tile_height_);
}
                               ...
                               std::vector<grid_tile*>& results) const
{
    ...
    else if (radius == 1)
    {
        for (int row_offset = -1; row_offset < 2; ++row_offset)
        {
            ...
            {
                int adj_col = (row_offset == -1 ? col_offset + 1 : col_offset);
                grid_tile* tmp_tile = get_tile_at(
                                                            origin.y + row_offset,
                                                            origin.x + adj_col);
                if (tmp_tile != NULL)
                {
                    results.push_back(tmp_tile);
                }
            }
        }
    }
    ...
    {
        results.push_back(get_tile_at(origin));
    }
}

void grid::get_intersections(const math::vector2f& a1,
                             const math::vector2f& a2,
                             std::vector<grid_tile*>& out) const
{
    for (unsigned row = 0; row < grid_vector_.size(); ++row)
    {
        for (unsigned col = 0; col < grid_vector_[row].size(); ++col)
        {
            if (grid_vector_[row][col]->get_occupant() != NULL)
            {
                sf::FloatRect bounds = grid_vector_[row][col]->get_bounds();
                // top
                math::vector2f b1(bounds.left, bounds.top);
                math::vector2f b2(bounds.left + bounds.width, bounds.top);
                if (math::line_intersect(a1, a2, b1, b2))
                {
                    out.push_back(grid_vector_[row][col]);
                    continue;
                }
                // right
                b1 = math::vector2f(bounds.left + bounds.width, bounds.top);
                b2 = math::vector2f(bounds.left + bounds.width, bounds.top + bounds.height);
                if (math::line_intersect(a1, a2, b1, b2))
                {
                    out.push_back(grid_vector_[row][col]);
                    continue;
                ...
                // bottom
                b1 = math::vector2f(bounds.left, bounds.top + bounds.height);
                b2 = math::vector2f(bounds.left + bounds.width, bounds.top + bounds.height);
                if (math::line_intersect(a1, a2, b1, b2))
                {
                    out.push_back(grid_vector_[row][col]);
                ...
                // left
                b1 = math::vector2f(bounds.left, bounds.top);
                b2 = math::vector2f(bounds.left, bounds.top + bounds.height);
                if (math::line_intersect(a1, a2, b1, b2))
                {
                    out.push_back(grid_vector_[row][col]);
                    continue;

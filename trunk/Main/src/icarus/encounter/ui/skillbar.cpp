    ...
    hero_(NULL)
{
    ...
    std::string bg_path = "images/ui/"+ui_root->child("skill_bar")->child("background")->value();
    ...
    {
        if (sf::Texture* bt_tex = resource_handler::get()->get_texture("images/ui/icons/"+ui_root->child("skill_bar")->child("default_icon")->value()))
        {
    ...
    portrait_.setTexture(*resource_handler::get()->get_texture("images/ui/icons/icon_empty.png"));
...
void skillbar::set_selected_hero(character_hero* hero)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        if (hero->get_skill(i).is_leveled() || hero->get_skill(i).is_ultimate())
        {
            buttons_[i]->set_disabled((hero->get_skill(i).get_level() == 0));
        }
        icon = "images/ui/icons/"+icon_root->child(hero_->get_skill(i).get_name())->value();

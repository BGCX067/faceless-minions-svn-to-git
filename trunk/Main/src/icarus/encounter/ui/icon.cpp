...
:
    ...
    cooldown_complete_(math::vector2f(0.0f, 0.0f),
                        resource_handler::get()->get_root_node("effects")->child("ui")->child("cooldown_complete"),
                        false, 0.0f),
    color_factor_(1.0f),
...
void icon::set_position(const sf::Vector2f pos)
{
    ...
    math::vector2f o_pos = pos;
    o_pos.x += icon_.getLocalBounds().width/2.0f;
    o_pos.y += icon_.getLocalBounds().height/2.0 - 5.0f;
    autocast_indicator_.set_world_pos(o_pos + math::vector2f(5.0f, 5.0f));
    cooldown_complete_.set_world_pos(o_pos + math::vector2f(0.0f, 5.0f));
